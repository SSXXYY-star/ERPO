...
class PointCloud_AE(nn.Module):
    def __init__(self, config):
        ...
        self.generator = DecoderInner(config.point_dim, config.latent_dim, config.hidden_dim, config.dec_layer_num,
                                      config.loss_type)
    ...
    def forward(self, inputs, z_vector, point_coord, is_training=False):
        if is_training:
            z_vector = self.encoder(inputs)  # Hs
            net_out = self.generator(point_coord, z_vector)  # O^hat_q
        else:
...
class VN_Resnet_Encoder(nn.Module):
    def __init__(self, hidden_dim, latent_dim, layer_num, num_k):
        ...
        self.layer_num = layer_num
        self.num_k = num_k
        self.conv_pos = VNLinearLeakyReLU(3, hidden_dim, negative_slope=0.2, share_nonlinearity=False,
                                          use_batchnorm=False)
...
class VN_DGCNN_Encoder(nn.Module):
    ...
    def forward(self, input):
        ...
        hidden = self.pool(hidden, dim=-1)  # 聚合k近邻邻居信息
        hiddens = []
