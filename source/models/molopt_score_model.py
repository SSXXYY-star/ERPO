...
import random
from source.models.common import ShiftedSoftplus, MLP, GaussianSmearing, find_closest_points, compute_bond_angle, compute_torsion_angle
from source.models.uni_transformer import UniTransformerO2TwoUpdateGeneral
from source.models.diffusion import *
...
class ScorePosNet3D(nn.Module):
    ...
    def forward(self, ligand_pos_perturbed, ligand_v_perturbed, 
        ...
        final_v = self.v_inference(final_h)  # eq11 MLP(h)
    ...
    def sample_diffusion(self, init_ligand_pos, init_ligand_v, batch_ligand, ligand_shape, 
        ...
        for i in tqdm(time_seq, desc='sampling', total=len(time_seq)):
            ...
            if use_mesh_data != None:
                ...
                pred_ligand_pos = preds_with_cond['pred_ligand_pos']
                if i > grad_step:
                    pred_ligand_pos = pointcloud_shape_guidance(use_pointcloud_data, pred_ligand_pos)  # SG
            ...
            with torch.no_grad():
                ...
                pos_model_mean = self.q_pos_posterior(x0=pos0_from_e, xt=ligand_pos, t=t, batch=batch_ligand)  # 计算P(xt-1)的概率分布
                pos_log_variance = extract(self.posterior_logvar, t, batch_ligand)
                # no noise when t == 0
                nonzero_mask = (1 - (t == 0).float())[batch_ligand].unsqueeze(-1)
                ligand_pos_next = pos_model_mean + nonzero_mask * (0.5 * pos_log_variance).exp() * torch.randn_like(
                    ligand_pos)  # 通过采样均值方差更新位置信息，不直接采用pred_ligand_pos

                ligand_pos = ligand_pos_next

                log_ligand_v_recon = F.log_softmax(v0_from_e, dim=-1)  # eq10 v = softmax(MLP(a))
                log_ligand_v = index_to_log_onehot(ligand_v, self.num_classes+int(self.v_mode=='tomask'))
                log_model_prob = self.q_v_posterior(log_ligand_v_recon, log_ligand_v, t, batch_ligand)  # 同样计算后验概率分布
                ligand_v_next = log_sample_categorical(log_model_prob)  # 采样原子特征
    ...
    @torch.no_grad()
    def sample_for_EGPO(self, init_ligand_pos, init_ligand_v, batch_ligand, ligand_shape, 
                         num_steps=None, center_pos_mode=None, use_mesh_data=None, use_pointcloud_data=None, 
                         use_pocket_data=None, grad_step=500, pred_bond=False, guide_stren=0, bounds=None, 
                         protein_pos=None, protein_ligand_dist_threshold=None):
        ...
        if self.config.shape_mode is not None:
            ligand_shape = ligand_shape.view(num_graphs, -1, 3)
        # traj: 各种随时间变化的运动轨迹信息
        pos_traj, v_traj = [init_ligand_pos], [init_ligand_v]  # policy input
        vt_pred_traj = [] # 第二个 calc ratio
        pos_mean_traj, pos_wo_pg_traj = [], []  # calc ratio
        ligand_pos, ligand_v = init_ligand_pos, init_ligand_v
        pred_bonds = None
        # time sequence
        time_seq = list(reversed(range(self.num_timesteps - num_steps, self.num_timesteps)))
        for i in time_seq:
            ...
            with torch.no_grad():
                ...
                pos_mean_traj.append(pos_model_mean.clone())
                pos_wo_pg_traj.append(ligand_pos.clone())
                ...
                # v0_pred_traj.append(log_ligand_v_recon.clone().cpu())
                vt_pred_traj.append(log_model_prob.clone())
                ligand_v = ligand_v_next
            ...
            pos_traj.append(ori_ligand_pos.clone())
            v_traj.append(ligand_v.clone())
        ...
        if pred_bond:
            ...
            return {
                'bond': pred_bonds,
                'pos_traj': pos_traj,
                'pos_mean_traj': pos_mean_traj,
                'pos_wo_pg_traj': pos_wo_pg_traj,
                'v_traj': v_traj,
                'vt_traj': vt_pred_traj
            }
        else:
    ...
    def get_one_step_with_grad(self, ligand_pos, ligand_v, batch_ligand, ligand_shape, pred_bond, time_step, guide_stren, 
                               center_pos_mode, num_graphs):
        t = torch.full(size=(num_graphs,), fill_value=time_step, dtype=torch.long, device=ligand_pos.device)
        if guide_stren != -1.0:
            preds_with_cond = self(
                ligand_pos_perturbed=ligand_pos,
                ligand_v_perturbed=ligand_v,
                ...
                ligand_shape=ligand_shape,
                if_test=True,
                pred_bond=True if self.pred_bond_type and pred_bond and time_step == 0 else False,
                time_step=t
            )
        
        if self.pred_bond_type and pred_bond and time_step == 0:
            pred_bonds = torch.argmax(preds_with_cond['bond_pred'], dim=-1).view(-1, 1)
            pred_edge_indices = preds_with_cond['edge_index'].transpose(0, 1)
            pred_bonds = torch.cat([pred_edge_indices, pred_bonds], dim=1)
        ...
        if center_pos_mode != 'none':
            ...
            ori_ligand_pos = ligand_pos
        pos_variance = extract(self.posterior_var, t, batch_ligand)
        return pos_model_mean, ligand_pos, vt_log_prob, ligand_v, pos_variance
...
def pointcloud_shape_guidance(use_pointcloud_data, pred_ligand_pos, k=3, ratio=0.2):
    ...
    while len(faratom_idxs) > 0 and j < 5:
        ...
        distance_scalar = np.expand_dims(np.random.random(faratom_points.shape[0]) * (0.8-ratio) + ratio, 1)  # 随机比例
        new_faratom_points = faratom_points - distance_scalar * distance_dir  # 更新过远原子的位置
    ...
    if j == 5:
        changed_faratom_idxs.extend(faratom_idxs)  # todo 是否可能导致过拟合？
...
def pocket_guidance(use_pocket_data, pred_ligand_pos, pred_ligand_v, step=0, k=3):
    ...
    while len(closeatom_idxs) > 0 and j < 5:
        ...
        distance_scalar = closeatom_dists + np.random.random(len(closeatom_dists)) * ratio  # 在原始距离上添加随机扰动 
