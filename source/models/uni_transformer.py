...
from source.utils.covalent_graph import connect_covalent_graph
import source.utils.data as utils_data
import time
import copy
from source.models.common import *
from source.models.shape_vn_layers import VNLinear, VNStdFeature, VNLinearLeakyReLU
import pdb
from source.utils.analyze import construct_bond_tensors
...
class BaseGVPAttLayer(nn.Module):
    ...
    def embed_ligand_shape(self, scalar_feat, vec_feat, ligand_shape, invar_ligand_shape):
        ...
        vec_feat_norm = norm_no_nan(vec_feat)  # eq18中的||r||
        try:
            net_shape = torch.einsum('bmi,bni->bmn', ligand_shape, vec_feat).view(N, -1)  # eq18中的<Hs, r>部分
            shape_input = torch.cat([scalar_feat, net_shape, vec_feat_norm, invar_ligand_shape], -1)
        except:
            pdb.set_trace()

        shape_emb = self.shape_emb_layer(shape_input)  # eq18 MLP
        return shape_emb
    ...
    def embed_message_att_weight(self, mess_scalar_emb, mess_vec_emb, node_scalar_emb, node_vec_emb, edge_feat, edge_index, r_feat, rel_x):  
        # mess_scalar_emb: m  mess_vec_emb: n  node_scalar_emb: a^  node_vec_emb: r^
        mess_vec_emb_norm = norm_no_nan(mess_vec_emb)
        
        src, dst = edge_index  # get i, j
        node_vec_emb_norm = norm_no_nan(node_vec_emb)

        kv_input = torch.cat([mess_scalar_emb, mess_vec_emb_norm], -1)  # eq14 [m, n]
        
        # compute k
        k = self.hk_func(kv_input)  # eq14 MLP get K
        # compute q
        q = self.hq_func(torch.cat([node_scalar_emb, node_vec_emb_norm], -1))  # eq14 get Q

        # compute attention weight
        att_weight = scatter_softmax((q[dst] * k / np.sqrt(k.shape[-1])).sum(-1), dst, dim=0)  # eq14 get e for weight
        
        return att_weight

    def message_passing(self, scalar_emb, vec_emb, x, ligand_emb, edge_feat, edge_index, r_feat, rel_x):
        N = scalar_emb.size(0)  # scalar_emb: a^  vec_emb: r^  x: x  ligand_emb: v noize atom feature  edge_feat: e for emb  r_feat: d  rel_x: xj - xi
        src, dst = edge_index  # get i j
        scalar_emb_i = scalar_emb[src]  # get a^_i
        vec_emb_i = vec_emb[src]  # get r^_i
        
        ## message embedding
        mess_scalar_i_in = torch.cat([scalar_emb_i, r_feat], -1)  # eq16 [a^, d]
        if edge_feat is not None and self.edge_feat_dim > 0:
            mess_scalar_i_in = torch.cat([mess_scalar_i_in, edge_feat], -1)  # eq16 get m^ = [a^, d, e]

        mess_vec_i_in = torch.cat([vec_emb_i, rel_x.unsqueeze(1)], -2)  # eq16 get n^ 
        #mess_scalar_j_in = scalar_emb_j
        #mess_vec_j_in = vec_emb_j

        mess_scalar_i_out, mess_vec_i_out = self.message_layer((mess_scalar_i_in, mess_vec_i_in))  # eq15 GVP get m, n

        ## attention weight
        att_weight = self.embed_message_att_weight(mess_scalar_i_out, mess_vec_i_out, scalar_emb, vec_emb, edge_feat, edge_index, r_feat, rel_x)  # eq14 get e for weight
        
        ## aggregate messages into atom embeddings
        w_mess_scalar = att_weight.unsqueeze(-1) * mess_scalar_i_out  # eq12 m * (e for weight)  加权
        w_mess_vec = att_weight.view(-1, 1, 1) * mess_vec_i_out  # eq12 n * (e for weight)  加权
        scalar_output = scatter_sum(w_mess_scalar, dst, dim=0, dim_size=N)  # 邻居信息求和
        vec_output = scatter_sum(w_mess_vec, dst, dim=0, dim_size=N)  # 邻居信息求和
        
        scalar_output = torch.cat([ligand_emb, scalar_emb, scalar_output], -1)  # eq12 get h

        vec_output = torch.cat([x, vec_emb, vec_output], -2)  # eq12 get y 

        scalar_output, vec_output = self.node_output_layer((scalar_output, vec_output))  # eq11 GVP get updated a, r
    ...
    def forward(self, scalar_feat, vec_feat, r_feat, rel_x, x, ligand_emb, edge_feat, edge_index, ligand_shape, invar_ligand_shape):
        # scalar_feat: a updated atom feature  vec_feat: r updated from x   r_feat: d atom distance expansion  rel_x: xj - xi atom distance  x: x atom_pos  ligand_emb: v noised atom feature  ligand_shape: Hs  invar_ligand_shape: VN-ln(Hs)
        # 从这里开始，到message passing是Shape-aware atom representation learning SARL过程
        o = self.embed_ligand_shape(scalar_feat, vec_feat, ligand_shape, invar_ligand_shape)  # eq18
        # update scalar feat with shape embedding o
        scalar_emb = self.shape_scalar_layer(torch.cat([scalar_feat, o], -1))  # eq17 get a^
        ...
        if self.use_shape_vec_mul:
            ...
            vec_emb_input = torch.cat([vec_feat, ligand_shape], 1)  # eq19 concat

        vec_emb = self.vector_emb_shape_layer(vec_emb_input)  # eq19 VN-MLP get r^
        
        # message passing
        scalar_output, vec_output = self.message_passing(scalar_emb, vec_emb, x, ligand_emb, edge_feat, edge_index, r_feat, rel_x)  # eq16 15 14 12 11 get updated a, r
...
class AttentionLayerO2TwoUpdateNodeGeneral(nn.Module):
    ...
    def _pred_edge_type(self, h, vec, x, edge_index, ligand_bond_index, ligand_bond_type, if_test=False):
        ...
        swap_edge_mask = torch.where(edge_index[0, :] >= edge_index[1, :])[0]  # 找到所有头index大于尾节点index的边
        swap_edge_mask_index = torch.index_select(edge_index[:, swap_edge_mask], 0, torch.LongTensor([1,0]).to(edge_index.device))  # 交换头尾
        unique_edge_index = torch.clone(edge_index)
        unique_edge_index[:, swap_edge_mask] = swap_edge_mask_index  # 获得心得edge index，其中所有头index大于尾index

        unique_pred_edge_index, unique_indices = torch.unique(unique_edge_index, sorted=True, return_inverse=True, dim=1)  # 去除重复的边 获得"去重后的唯一边集合"，"原始每条边在唯一边集合中的索引"
        ...
        if self.output_dim[1] == 1:
            ...
            src, dst = unique_pred_edge_index  # get i j
            norm1, norm2 = norm_no_nan(vec[src]), norm_no_nan(vec[dst])  # get ri rj
            sum_vec_norm = norm1 + norm2  # ri + rj
            diff_vec_norm = torch.abs(norm1 - norm2)  # abs(ri - rj)
            diff_h = torch.abs(h[src] - h[dst])  # abs(ai - aj)
            sum_h = h[src] + h[dst]  # ai + aj
            bond_input = torch.cat((diff_vec_norm, sum_vec_norm, diff_h, sum_h), dim=1)  # eq20 concat
            bond_pred = self.b_inference(bond_input)  # eq20 MLP get e for edge
    ...
    def forward(self, h, vec, x, ligand_emb, edge_attr, edge_index, ligand_bond_index, ligand_bond_type, \
        ...
        # h: a updated atom feature  vec: r updated from x x.unsequeeze  x: x atom_pos  ligand_emb: v noised atom feature  ligand_shape_emb: Hs  invar_ligand_shape: VN-ln(Hs)
        ...
        new_h, new_vec = self.gvp_layer(h_in, vec, dist_feat, rel_x, x.unsqueeze(1), 
                                ...
                                invar_ligand_shape)  # from eq11 get a, r
...
class UniTransformerO2TwoUpdateGeneral(nn.Module):
    ...
    def _connect_graph(self, ligand_pos, ligand_v, batch):
        edge_index = self._connect_edge(ligand_pos, ligand_v, batch)  # KNN作为边
        ...
        if self.cutoff_mode == 'cov_radius':
            ...
            edge_type = self._build_edge_type(ligand_v, ligand_pos, edge_index)  # KNN 找到边，通过v获取原子类型，通过原子类型以及距离判断键类型
    ...
    def forward(self, v, h, x, batch_ligand, ligand_shape=None, pred_bond=False, ligand_bond_index=None, ligand_bond_type=None, if_test=False, return_all=None):
        # v: v ligand_v_perturbed  h: a ligand_atom_emb 扰动的原子v与时间concat并mlp  x: x ligand_pos_perturbed  ligand_shape: Hs 
        ...
        if ligand_shape is not None:
            invar_ligand_shape_emb = self.invariant_shape_layer(ligand_shape)  # eq18中的VN-ln(Hs)
