...
from source.utils.tfbio_data import make_grid, ROTATIONS
import random
import copy
from rdkit.Chem import rdMolTransforms
from rdkit import Geometry
from source.utils.tfbio_data import rotation_matrix
...
def get_atom_stamp(grid_resolution, max_dist):
    """
        获取所有原子的模板。模板根据vdw半径计算。依据分辨率以及最大距离，得到一个三维立方体矩阵，半径外数据为0，半径内数据为原子序数。
...
def get_voxel_shape(mol, atom_stamp, grid_resolution, max_dist, confId=-1, without_H=True, by_coords=False, coords=None, features=None):
    # expand each atom point to a sphere
    # 该函数在仅包含原子质心的3D网格的基础上，依照原子半径等信息进一步扩展网格中的信息，即 网格中以原子质心为中心，原子半径为半径的所有网格内元素均为1。
    if not by_coords:
        coords, features = get_binary_features(mol, confId, without_H)  # 特征是原子类型，coords是坐标
    grid, atomic2grid = make_grid(coords, features, grid_resolution, max_dist)  # grid: 1, 45, 45, 45, 1 表示一个三维网格中是否有原子在点上。如果在，则该点为特征，否则为0；atomic2grid: 字典，表示每个类型原子所在三维网格坐标
    shape = np.zeros(grid[0, :, :, :, 0].shape)
    for tup in atomic2grid:
        atomic_number = int(tup[0])
        stamp = atom_stamp[ATOMIC_NUMBER_REVERSE[atomic_number]]
...
def get_mesh(mol, center_pos=False, confId=-1, scaling=1.0, probe_radius=1.4):
    ...
    oddtconftool.calccharges()  # 为原子分配部分电荷
    # 通过Marching Cubes算法将分子表面离散化为3D网格，返回vertices，N*3表示顶点坐标，triangles，M*3表示三角面片顶点索引
...
def get_pointAE_shape_emb(mols, model, point_cloud_samples, config, shape_parallel=False, batch_size=32):
    ...
    # None Hs bounds point_clouds point_cloud_centers
