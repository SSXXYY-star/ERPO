...
import copy
from source.utils.data import *
from .shape_mol_data import ShapeMolData, torchify_dict
from source.utils.shape import *
from functools import partial
from multiprocessing import Pool
from source.utils.subproc_shapeAE import SubprocShapeAE
...
class ShapeMolDataset(Dataset):
    ...
    def _process_mose(self, db):
        shape_func, subproc_voxelae = get_shape_func(self.config.shape)  # 获取AE模型以及过程函数
        ...
        with db.begin(write=True, buffers=True) as txn:
            if 'test' in self.raw_path:
                ...
                all_mols = pickle.load(open(self.raw_path, 'rb'))['rdkit_mol_cistrans_stereo'][:10000]  # TODO : 仅用前1w个数据测试
...
def get_shape_func(config):
    if config.shape_type == 'electroshape':
        ...
        if config.shape_parallel: shapeae = SubprocShapeAE(config)
        else: shapeae = build_point_shapeAE_model(config, device='cuda')  # 读取AE模型并使参数不计算梯度
